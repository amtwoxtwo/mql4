...
int start()
  {
   bool SpreadIsNormal=true;
//---
   if((Ask-Bid)/pt>LimitSpread) SpreadIsNormal=false;
//---
            ...
            if(OrderType()==type)
              {
               if(OrderStopLoss()!=s_l || OrderTakeProfit()!=t_p)
                 {
                  ModifiedOrder=OrderModify(OrderTicket(),OrderOpenPrice(),s_l,t_p,0,CLR_NONE);
                 }
              }
           }
        }
...
void closeall()
  {
   ...
   bool CloseedOrder=false;
   for(int i=OrdersTotal()-1; i>=0; i--)
     {
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
        {
         if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=magic) continue;
         if(OrderType()>1) DeletedOrder=OrderDelete(OrderTicket());
         else
           {
            if(OrderType()==0) CloseedOrder=OrderClose(OrderTicket(),OrderLots(),Bid,3,CLR_NONE);
            else               CloseedOrder=OrderClose(OrderTicket(),OrderLots(),Ask,3,CLR_NONE);
           }
        }
     }
  }
